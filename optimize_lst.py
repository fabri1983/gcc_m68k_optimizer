...
# How many bytes the short branch does span. Subtract 2 to account for the branch instruction short size.
...
# Unconditional instructions. We only want to capture all the different dN/aN registers.
...
# Use next pattern with .findall(). We only want to capture all the different dN/aN registers.
REG_AS_SOURCE_OR_INDIRECT_USE_REGEX = re.compile(
    r'-?\((%a[0-7])\)\+?'            # Indirect decrement/increment addressing register as [-](aN)[+]
    r'|'
    r'\((?:%pc|%sp),(%[ad][0-7])(?:\.[bwl])?\)'  # Indirect addressing register as (pc/sp,xN.s)
    r'|'
    r'\((%a[0-7]),(%[ad][0-7])(?:\.[bwl])?\)'  # Indirect addressing registers as (aN,xN[.s])
    r'|'
...
REG_OVERWRITEN_OR_CLEARED_REGEX = re.compile(
    ...
    r'('                             # Capturing group for every alternative
        r'(?:\((?:%a[0-7]|%sp|%pc)\))'  # (aN/pc/sp)
        r'|'
        r'(?:\((?:%a[0-7]|%sp|%pc),(?:%[ad][0-7](?:\.[bwl])?|%sp)\))'  # (aN/PC,xN.s)
        r'|'
        r'(?:(?:-?[0-9a-zA-Z_\.]+)(?:\.[bwl])?(?:[\-\+\*]\d+)?\((?:%a[0-7]|%sp|%pc),(?:%[ad][0-7](?:\.[bwl])?|%sp)\))'  # label_or_disp[+-*N](aN/PC,xN.s)
        r'|'
        r'(?:(?:-?[0-9a-zA-Z_\.]+)(?:\.[bwl])?(?:[\-\+\*]\d+)?\((?:%a[0-7]|%sp|%pc)\))'  # label_or_disp[+-*N](aN/pc/sp)
        r'|'
        r'(?:(?:-?[0-9a-zA-Z_\.]+)(?:\.[bwl])?(?:[\-\+\*]\d+)?)'  # label_or_disp[+-*N]
        # TODO: capturing everything before a comma might be dangerous if we didn't consider explicit matches before
        r'|'
...
class ControlFlowPosInArray:
    ...
    label_pos_in_lines: int = -1
    """ Position in modified_lines where the label is defined"""
    label_pos_in_modified_lines: int = -1
...
def build_control_flow_map(i_line: int, lines: list[str], modified_lines: list[str]) -> dict[str, ControlFlowPosInArray]:
    """
    Only for current subroutine scope.
    Allows tracking of code flow from any jump/branch instruction.
    Special number labels (eg: 0f, 2f) are already renamed to a proper naming convention by another function.
    Builds dictionary:
        {
            key: label,
            value: {
                label_pos_in_lines: int,
                label_pos_in_modified_lines: int,
    ...
    Returns: control_flow_dict to be accessed as:
        ...
        control_obj.label_pos_in_lines
        control_obj.label_pos_in_modified_lines
    ...
    for i in range(start_idx, end_idx - 1, -1):  # backwards
        ...
        if match := LABEL_REGEX.match(line):
            ...
            control_obj = ControlFlowPosInArray(label_pos_in_lines=-1, label_pos_in_modified_lines=i)
    ...
    for i in range(rem_start, rem_end):  # forwards
        ...
        if match := LABEL_REGEX.match(line):
            label = match.group(1)
            # Save the line position of the label
            control_obj = ControlFlowPosInArray(label_pos_in_lines=i, label_pos_in_modified_lines=-1)
            control_flow_dict[label] = control_obj
...
def get_function_name(i_line: int, lines: list[str]) -> str:
    """
    Get the funciton name of current subroutine.
    """
    start_idx = i_line - 1
    end_idx = 0
    func_name = ""
...
@export_func
def find_free_after_use_address_register(excludes: list[str], i_line: int, lines: list[str], modified_lines: list[str], ignore_N_previous_lines: int=0) -> list[str | None]:
    excludes.append("%a7")
...
def find_free_after_use_register(excludes: list[str], i_line: int, lines: list[str], modified_lines: list[str], reg_type: str, ignore_N_previous_lines: int) -> list[str | None]:
    ...
    for i in range(start_idx, end_idx - 1, -1):  # backwards
        ...
        # Skip comments
        if line[0] in COMMENT_PREFIX_CHAR:
            continue
    ...
    while True:
                ...
                for reg_str in regs_list:
                    if reg_str.startswith(reg_type):
                        ...
                        if (reg_index not in exclude_indexes) and not (overwritten_or_cleared_mask & (1 << reg_index)):
                            ...
                            candidate_mask &= ~(1 << reg_index) & 0xFF  # Mark candidate as not available
        ...
        if target_array is modified_lines and i == rem_end:
            ...
            target_array = lines
            rem_end = len(target_array)
        # If there is any return frame left, then continue from that location
        elif len(flow_return_frames) > 0:
    ...
    if candidate_mask:
        ...
        while candidate_mask:
            lsb = candidate_mask & -candidate_mask
            bit_index = lsb.bit_length() - 1
            candidates.append(f'{reg_type}{bit_index}')
            candidate_mask ^= lsb
...
@export_func
def find_unused_data_register(excludes: list[str], i_line: int, lines: list[str], modified_lines: list[str], ignore_N_previous_lines: int=0) -> list[str | None]:
    return find_unused_register(excludes, i_line, lines, modified_lines, "%d", ignore_N_previous_lines)

@export_func
def find_unused_address_register(excludes: list[str], i_line: int, lines: list[str], modified_lines: list[str], ignore_N_previous_lines: int=0) -> list[str | None]:
    excludes.append("%a7")
    return find_unused_register(excludes, i_line, lines, modified_lines, "%a", ignore_N_previous_lines)

def find_unused_register(excludes: list[str], i_line: int, lines: list[str], modified_lines: list[str], reg_type: str, ignore_N_previous_lines: int) -> list[str | None]:
    """
    Search for unused registers from the beginning of the routine until i_line position, considering
    all possible flows.
    Unused regs, other than regs in excludes array, are those not used as target operand (ie the reg 
    will be used later on) nor source/indirect operand.
    ...
    while True:
        ...
        while i < rem_end:  # forwards
            ...
            # Operating over lines array and reaching i_line+1?
            if target_array is lines and i-1 == i_line+1:
                break  # Stop the analysis at current flow
            ...
            if PUSH_REGS_INTO_STACK_REGEX.match(line):
                continue
...
def in_a_SGDK_sound_related_routine(modified_lines: list[str]) -> bool:
    ...
    for i in range(start_idx, end_idx - 1, -1):
        ...
        if match := FUNCTION_DECLARATION_REGEX.match(line):
            return match.group(1).startswith(('Z80_','XGM_','XGM2_','SND_','PSG_','YM2612_','MDS_'))
    ...
    for i in range(rem_start, rem_end):  # forwards
        ...
        if xN_overwritten_or_cleared:
            ...
            # Is it a movem/move pop for xN?
                ...
                is_before_rts_rte = True if FUNCTION_EXIT_REGEX.match(lines[i+1]) else False
                if is_before_rts_rte and xN in extract_registers(pop_match.group(3), POP_OP):
                    collected_indices.append(i)
    ...
    for i in collected_indices:
        ...
        if pop_match := POP_REGS_FROM_STACK_REGEX.match(line):
            ...
            if pop_match.group(1) == 'move':
                ...
                if is_before_rts_rte and (inAnInterruptRoutine or (xM not in scratch_pad)):
...
def add_regs_into_push_pop_if_not_scratch_or_in_interrupt(regs_to_add: list[str], i_line: int, lines: list[str], modified_lines: list[str]) -> bool:
    ...
    if regs_added_count > 0:
        ...
        print("Add regs into movem/move pop. Might need SP use adjustments")
...
def optimizeMultiLines_8 (i_line: int, lines: list[str], modified_lines: list[str], current_pass: int) -> tuple[list[str] | None, int]:
    ...
    if matchA:
        ...
        if dispA == 0:
            ...
            if matchB and s == matchB.group(2) and aN == matchB.group(5) and aM == matchB.group(6):
                ...
                if dispB == dispA+stride:
                    ...
                    if matchC and s == matchC.group(2) and aN == matchC.group(5) and aM == matchC.group(6):
                        ...
                        if dispC == dispB+stride:
                            ...
                            if matchD and s == matchD.group(2) and aN == matchD.group(5) and aM == matchD.group(6):
                                ...
                                if dispD == dispC+stride:
                                    ...
                                    if matchE and s == matchE.group(2) and aN == matchE.group(5)and aM == matchE.group(6):
                                        ...
                                        if dispE == dispD+stride:
                                            ...
                                            if matchF and s == matchF.group(2) and aN == matchF.group(5) and aM == matchF.group(6):
                                                ...
                                                if dispF == dispE+stride:
                                                    ...
                                                    if matchG and s == matchG.group(2) and aN == matchG.group(5) and aM == matchG.group(6):
                                                        dispG = 0 if not matchG.group(4) else parseConstantSigned(matchG.group(4), 16)
                                                        if dispG == dispF+stride:
                                                            matchH = re.match(r'^\s*lea\s+(-?\d+)\((%a[0-7])\),\s*(%a[0-7])', line_H)
                                                            if matchH and aN == matchH.group(2) and aN == matchH.group(3):
                                    ...
                                    if matchE and s == matchE.group(2) and aN == matchE.group(5) and aM == matchE.group(6):
                                        ...
                                        if dispE == dispD+stride:
                                            ...
                                            if matchF and s == matchF.group(2) and aN == matchF.group(5) and aM == matchF.group(6):
                                                dispF = 0 if not matchF.group(4) else parseConstantSigned(matchF.group(4), 16)
                                                if dispF == dispE+stride:
                                                    matchG = re.match(r'^\s*lea\s+(-?\d+)\((%a[0-7])\),\s*(%a[0-7])', line_G)
                                                    if matchG and aN == matchG.group(2) and aN == matchG.group(3):
                            ...
                            if matchD and s == matchD.group(2) and aN == matchD.group(5) and aM == matchD.group(6):
                                ...
                                if dispD == dispC+stride:
                                    ...
                                    if matchE and s == matchE.group(2) and aN == matchE.group(5) and aM == matchE.group(6):
                                        dispE = 0 if not matchE.group(4) else parseConstantSigned(matchE.group(4), 16)
                                        if dispE == dispD+stride:
                                            matchF = re.match(r'^\s*lea\s+(-?\d+)\((%a[0-7])\),\s*(%a[0-7])', line_F)
                                            if matchF and aN == matchF.group(2) and aN == matchF.group(3):
                ...
                if matchC and dN == matchC.group(1) and aN == matchC.group(2):
                    ...
                    if matchD and aN == matchD.group(3):
                        ...
                        if matchE and aN == matchE.group(2):
                            ...
                            dM = matchE.group(3)
                            matchF = re.match(r'^\s*move\.l\s+(%d[0-7]),\s*(-?\d+|0[xX][0-9a-fA-F]+)?\((%a[0-7]|%sp)\)', line_F)
...
def optimizeMultiLines_5 (i_line: int, lines: list[str], modified_lines: list[str], current_pass: int) -> tuple[list[str] | None, int]:
                    ...
                    if matchC and s == matchC.group(2) and aN == matchC.group(5) and aM == matchC.group(6):
                        ...
                        if dispC == dispB+stride:
                            ...
                            if matchD and s == matchD.group(2) and aN == matchD.group(5) and aM == matchD.group(6):
                                dispD = 0 if not matchD.group(4) else parseConstantSigned(matchD.group(4), 16)
                                if dispD == dispC+stride:
                                    matchE = re.match(r'^\s*lea\s+(-?\d+)\((%a[0-7])\),\s*(%a[0-7])', line_E)
                                    if matchE and aN == matchE.group(2) and aN == matchE.group(3):
    ...
    if USE_COOL_OPTIMIZATIONS:
        ...
        # Unnecessary clear of data register to multiply an address register by 2 and add/sub a constant.
...
def optimizeMultiLines_4 (i_line: int, lines: list[str], modified_lines: list[str], current_pass: int) -> tuple[list[str] | None, int]:
            ...
            if matchB and s == matchB.group(2) and aN == matchB.group(5) and aM == matchB.group(6):
                ...
                if dispB == dispA+stride:
                    ...
                    if matchC and s == matchC.group(2) and aN == matchC.group(5) and aM == matchC.group(6):
                        dispC = 0 if not matchC.group(4) else parseConstantSigned(matchC.group(4), 16)
                        if dispC == dispB+stride:
                            matchD = re.match(r'^\s*lea\s+(-?\d+)\((%a[0-7])\),\s*(%a[0-7])', line_D)
            ...
            if matchB and matchC:
                ...
                if sA == sB == sC:
                    ...
                    if are_same_aregs and are_consecutive_disps:
                        ...
                        xregs = [matchA.group(7), matchB.group(7), matchC.group(7)]
        ...
        if matchA:
            ...
            if matchB and matchC and matchD:
                ...
                if sA == sB == sC == sD:
                    ...
                    are_first_three_xregs_sorted = are_regs_sorted([matchA.group(7), matchB.group(7), matchC.group(7)])
                    are_last_three_xregs_sorted = are_regs_sorted([matchB.group(7), matchC.group(7), matchD.group(7)])

                    if are_same_aregs and (are_first_three_xregs_sorted or are_last_three_xregs_sorted):

                        disps = [dispA, dispB, dispC, dispD]
                        xregs = [matchA.group(7), matchB.group(7), matchC.group(7), matchD.group(7)]
    ...
    if USE_COOL_OPTIMIZATIONS:
        ...
        # sub*.s  #2,sp                        move.w    symbol_or_mem,-4(sp)
        # move.w  symbol_or_mem,-(sp)          subq.s    #6,sp
        # sub*.s  #2,sp
        # Where:
        #  symbol_or_mem[.wl][+-*N][.bwl]
        matchA = re.match(r'^(\s*)move\.w(\s+)(-?[0-9a-zA-Z_\.]+)(\.[wl])?([\-\+\*]\d+)?(\.[bwl])?,\s*-\(%sp\)', line_A)
        ...
        if matchA:
            ...
            if matchB and s == matchB.group(2) and aN == matchB.group(4):
                alu = matchB.group(1)
                val = matchB.group(3)
                matchC = re.match(r'^\s*move\.([wl])\s+(%a[0-7]),\s*(-?\d+|0[xX][0-9a-fA-F]+)?\((%a[0-7]|%sp)\)', line_C)
        ...
        # Unnecessary clear of data register to multiply an address register by 2.
        ...
        # swap    dN
        # move.l  dN,dM
        # Leaves dN with different value in upper word. 
        # Ensure dN is not used before is cleared or overwritten, or is used as word or byte.
                ...
                if matchC_clr or matchC_pea:
                    ...
                    if matchD_clr or matchD_pea:
                        ...
                        if len(free_d_regs) < 4:
                            existent_candidates = [] if free_d_regs[0] == None else free_d_regs
                            free_d_regs = find_unused_data_register(existent_candidates, i_line, lines, modified_lines, 4)
                            free_d_regs = existent_candidates + [r for r in free_d_regs if r]  # Discards None reg
                        if len(free_d_regs) >= 4:
...
def optimizeMultiLines_3 (i_line: int, lines: list[str], modified_lines: list[str], current_pass: int) -> tuple[list[str] | None, int]:
    ...
    if USE_COOL_OPTIMIZATIONS:
        ...
        # Unnecessary redundant use of address register.
        ...
        # Unnecessary redundant use of data register.
        # move.b    (aN),dM    ->    instr.b   (aN),dN     ; Saves 4 cycles. Leaves dM free.
        # and.w     #255,dM          and.w     #255,dN
        # instr.b   dM,dN
        # Where instr is one of next for .b accessor: or|eor|add|sub
        # Leaves dM free. Ensure dM is not used before is cleared or overwritten.
        matchA = re.match(r'^(\s*)move\.b(\s+)\((%a[0-7]|%sp)\),\s*(%d[0-7])', line_A)
        if matchA:
            aN = matchA.group(3)
            dM = matchA.group(4)
            matchB = re.match(r'^\s*(and|andi)\.w\s+#(-?\d+|0[xX][0-9a-fA-F]+),\s*(%d[0-7])', line_B)
            if matchB and dM == matchB.group(3):
                mask = parseConstantUnsigned(matchB.group(2))
                if mask == 0xFF:
                    instr_and = matchB.group(1)
                    matchC = re.match(r'^\s*(or|eor|add|sub)\.b\s+(%d[0-7]),\s*(%d[0-7])', line_C)
        ...
        # Clean word before copying a byte.
        # move.b    (aN),dN    ->    moveq     #0,dN       ; Saves 4 cycles. Leaves dN different.
        # and.w     #255,dN          move.b    (aN),dN
        # instr.w   dM,dN            instr.w   dM,dN
        # Where instr is one of next for .w accessor: or|eor|add|sub|muls|mulu|divs|divu.
        # Leaves dN different. Ensure dN is not used anymore, or is used as byte or word before being cleared or overwritten.
        matchA = re.match(r'^(\s*)move\.b(\s+)\((%a[0-7]|%sp)\),\s*(%d[0-7])', line_A)
        if matchA:
            aN = matchA.group(3)
            dN = matchA.group(4)
            ...
            if matchB and dN == matchB.group(3):
                mask = parseConstantUnsigned(matchB.group(2))
                if mask == 0xFF:                
                    matchC = re.match(r'^\s*(or|eor|add|sub|muls|mulu|divs|divu)\.w\s+(%d[0-7]),\s*(%d[0-7])', line_C)
                    if matchC and dN == matchC.group(3):
                        ...
                        if not is_reg_used_before_overwrite_or_clear or only_used_as_word_or_byte_before_overwrite_or_clear:
                            optimized_lines = [
                                f'{matchA.group(1)}moveq{matchA.group(2)}#0,{dN}',
                                f'{matchA.group(1)}move.b{matchA.group(2)}({aN}),{dN}',
                                f'{matchA.group(1)}{instr_C}.w{matchA.group(2)}{dM},{dN}'
                            ]
                            return (optimized_lines, 3)
            ...
            if matchB_clr or matchB_pea:
                ...
                if matchC_clr or matchC_pea:
                    ...
                    if len(free_d_regs) < 3:
                        existent_candidates = [] if free_d_regs[0] == None else free_d_regs
                        free_d_regs = find_unused_data_register(existent_candidates, i_line, lines, modified_lines, 3)
                        free_d_regs = existent_candidates + [r for r in free_d_regs if r]  # Discards None reg
                    if len(free_d_regs) >= 3:
...
def optimizeMultiLines_2 (i_line: int, lines: list[str], modified_lines: list[str], current_pass: int) -> tuple[list[str] | None, int]:
    ...
    # lsl.l  #val,dN          lsl.w  #val,dN
    matchA = re.match(r'^(\s*)(and|andi)\.l(\s+)#(-?\d+|0[xX][0-9a-fA-F]+),\s*(%d[0-7])', line_A)
    if matchA:
        mask = parseConstantUnsigned(matchA.group(4))
        dN = matchA.group(5)
        if mask == 0xFF:
            matchB = re.match(r'^\s*lsl\.l\s+#(-?\d+|0[xX][0-9a-fA-F]+),\s*(%d[0-7])', line_B)
            if matchB and dN == matchB.group(2):
                val = parseConstantUnsigned(matchB.group(1))
                if 1 <= val <= 8:
                    optimized_lines = [
                        f'{matchA.group(1)}and.l{matchA.group(3)}#255,{dN}',
                        f'{matchA.group(1)}lsl.w{matchA.group(3)}#{val},{dN}'
                    ]
                    return (optimized_lines, 2)
    ...
    if matchA:
        ...
        if matchB and aM == matchB.group(1):
            val = '' if not matchA.group(3) else matchA.group(3)
            if_reg_not_used_anymore_then_remove_from_push_pop(aM, i_line, lines, modified_lines, 2)
            optimized_lines = [
                f'{matchA.group(1)}jmp{matchA.group(2)}{val}({aN_or_pc})'
            ]
            return (optimized_lines, 2)
    ...
    if matchA:
        ...
        if matchB and aM == matchB.group(1):
            ...
            if_reg_not_used_anymore_then_remove_from_push_pop(aM, i_line, lines, modified_lines, 2)
            optimized_lines = [
                f'{matchA.group(1)}jmp{matchA.group(2)}{val}({aN_or_pc},{dN_s})'
            ]
            return (optimized_lines, 2)
    ...
    if matchA:
        ...
        if matchB and dN == matchB.group(4):
            ea = next((matchA.group(i) for i in range(4, 10) if matchA.group(i)), None)
    ...
    if USE_COOL_OPTIMIZATIONS:
        ...
        matchA = re.match(r'^(\s*)lea(\s+)(-?\d+|0[xX][0-9a-fA-F]+)?\((%a[0-7])\),\s*(%a[0-7])', line_A)
        if matchA:
            disp_str = matchA.group(3)
    ...
    if matchA:
        ...
        # If -32767 <= val <= 32767
        # move.s  aN,aM      ->    lea   val(aN),aM        ; Saves [8,12] cycles.
        # add.s   #val,aM
        # s: b,w,l.
        matchB = re.match(r'^\s*(add|adda|addq)\.([bwl])\s+#(-?\d+|0[xX][0-9a-fA-F]+),\s*(%a[0-7]|%sp)', line_B)
    ...
    add_disp_aN_into_dN_pattern = r'^(\s*)add\.([bwl])(\s+)(-?\d+|0[xX][0-9a-fA-F]+)?\((%a[0-7])\),\s*(%d[0-7])'
    ...
    #                            sub.s   dP,dM
    # Needs a free data register dP.
    sub_disp_aN_into_dN_pattern = r'^(\s*)sub\.([bwl])(\s+)(-?\d+|0[xX][0-9a-fA-F]+)?\((%a[0-7])\),\s*(%d[0-7])'
    ...
    add_disp_aN_into_aM_pattern = r'^(\s*)(?:add|adda)\.([bwl])(\s+)(-?\d+|0[xX][0-9a-fA-F]+)?\((%a[0-7])\),\s*(%a[0-7])'
            ...
            if dispA == dispB:
                ...
                if aQ is not None:
                    if add_regs_into_push_pop_if_not_scratch_or_in_interrupt([aQ], i_line, lines, modified_lines):
                        optimized_lines = [
                            f'{matchA.group(1)}move.{s}{matchA.group(3)}{disp_str}({aN}),{aQ}',
                            f'{matchA.group(1)}add.{s} {matchA.group(3)}({aQ}),{aM}',
                            f'{matchA.group(1)}add.{s} {matchA.group(3)}({aQ}),{aP}'
                        ]
                        return (optimized_lines, 2)
    ...
    #                            sub.s   aQ,aP
    # Needs a free address register aQ.
    sub_disp_aN_into_aM_pattern = r'^(\s*)(?:sub|suba)\.([bwl])(\s+)(-?\d+|0[xX][0-9a-fA-F]+)?\((%a[0-7])\),\s*(%a[0-7])'
    ...
    if matchA:
        ...
        if matchB and s == matchB.group(2) and aN == matchB.group(5):
            dispA = 0 if not matchA.group(4) else parseConstantSigned(matchA.group(4), 16)
            dispB = 0 if matchB.group(4) else parseConstantSigned(matchB.group(4), 16)
            # Must have same displacement
            if dispA == dispB:
                disp_str = '' if dispA == 0 else str(dispA)
                aP = matchB.group(6)
                ...
                if aQ is not None:
                    if add_regs_into_push_pop_if_not_scratch_or_in_interrupt([aQ], i_line, lines, modified_lines):
                        optimized_lines = [
                            f'{matchA.group(1)}move.{s}{matchA.group(3)}{disp_str}({aN}),{aQ}',
                            f'{matchA.group(1)}sub.{s} {matchA.group(3)}({aQ}),{aM}',
                            f'{matchA.group(1)}sub.{s} {matchA.group(3)}({aQ}),{aP}'
                        ]
                        return (optimized_lines, 2)
    ...
    if matchA:
        ...
        # neg.s    dN         ->    add.s   dN,dM       ; Saves 4 cycles. Leaves dN different.
        # sub.s    dN,dM
        # Leaves dN with different value than expected.
        # Ensure dN is not used before is overwritten or cleared.
    ...
    if matchA:
        ...
        if matchB:
            ...
            symbolName_2_op = 0 if not op_2_str else int(op_2_str)
            ...
            if matchA.group(2) == 'w' and matchB.group(2) == 'w':
                if symbolName_1 == symbolName_2 and (symbolName_1_op + 2 == symbolName_2_op):
                    optimized_lines = [
                        f'{matchA.group(1)}clr.l{matchA.group(3)}{symbolName_1}{op_1_str}'
                    ]
                    return (optimized_lines, 2)

            # If clearing symbolName and symbolName-2.
            # clr.w   symbolName       ->    clr.l   symbolName-2  ; Saves 12 cycles.
            # clr.w   symbolName-2
            if matchA.group(2) == 'w' and matchB.group(2) == 'w':
                if symbolName_1 == symbolName_2 and (symbolName_1_op - 2 == symbolName_2_op):
                    optimized_lines = [
                        f'{matchA.group(1)}clr.l{matchA.group(3)}{symbolName_1}{op_2_str}'
                    ]
                    return (optimized_lines, 2)
    ...
    if matchA:
        ...
        if matchB:
            ...
            # If mem1-2 == mem2
            # clr.w   mem1       ->    clr.l   mem2        ; Saves 12 cycles.
            # clr.w   mem2
            if matchA.group(2) == 'w' and matchB.group(2) == 'w':
                if mem1-2 == mem2:
                    s_mem = '' if not matchB.group(5) else matchB.group(5)
                    optimized_lines = [
                        f'{matchA.group(1)}clr.l{matchA.group(3)}{mem2}{s_mem}'
                    ]
                    return (optimized_lines, 2)

    # Clearing consecutive memory calculated from effective address.
    clr_mem_ea_pattern = r'^(\s*)clr\.([bw])(\s+)(-?\d+|0[xX][0-9a-fA-F]+)?\((%a[0-7])\)'
        ...
        if matchB:
            ...
            if matchA.group(2) == 'w' and matchB.group(2) == 'w':
                ...
                if disp1+2 == disp2 and aN == matchB.group(5):
                    disp1_str = '' if disp1 == 0 else str(disp1)
                    optimized_lines = [
                        f'{matchA.group(1)}clr.l{matchA.group(3)}{disp1_str}({aN})'
                    ]
                    return (optimized_lines, 2)

            # If disp1-2 == disp2
            # clr.w   disp1(aN)      ->    clr.l   disp2(aN)       ; Saves 8 cycles.
            # clr.w   disp2(aN)
            if matchA.group(2) == 'w' and matchB.group(2) == 'w':
                disp1 = 0 if not matchA.group(4) else parseConstantSigned(matchA.group(4), 32)
                disp2 = 0 if not matchB.group(4) else parseConstantSigned(matchB.group(4), 32)
                ...
                if disp1-2 == disp2 and aN == matchB.group(5):
                    disp2_str = '' if disp2 == 0 else str(disp2)
                    optimized_lines = [
                        f'{matchA.group(1)}clr.l{matchA.group(3)}{disp2_str}({aN})'
                    ]
                    return (optimized_lines, 2)
...
def optimizeSingleLine_Peepholes(line: str, i_line: int, lines: list[str], modified_lines: list[str]) -> tuple[list[str], bool]:
    ...
    if match:
        optimized_line = indirection_0_pattern.sub(r'(\1)', line)
        return ([optimized_line], True)
...
# acá: this breaks sprites on Metal Slug project
    ...
    if match:
        ...
        # move.l  #val,dN  ->   moveq    #m,dN         ; Saves 4 cycles.
        #                       swap     dN
        ...
        #   - MOVEQ for 16-bit values where $FF81 <= x <= $FFFF (Saves 8 cycles)
        #   and
        #   - MOVEQ+NEG.W for 32-bit values where $FFFF0001 <= x <= $FFFF0080 (Saves 4 cycles)
        # Explanation:
        ...
        # move.l  #$N0000,Dn   ->   moveq    #N,Dn     ; Saves 4 cycles.
        #                           swap     Dn
        ...
        #                                      -128 <= N <= -2
        # The moveq instruction sign extends the last bit.
        # move.l  #$NFFFF,Dn   ->   moveq    #N,Dn     ; Saves 4 cycles.
        #                           swap     Dn
...
# acá: this breaks player sprite on Metal Slug project
    ...
    match = re.match(r'^(\s*)(move|movea)\.l(\s+)#0,\s*(%a[0-7]|%sp)', line)
    if match:
        aN = match.group(4)
    ...
    # add*.s  #0,dN       ->   tst.s  dN          ; Saves [0,4,12] cycles.
    ...
    # sub*.s  #0,dN       ->   tst.s  dN          ; Saves [0,4,12] cycles.
    ...
    match = re.match(r'^(\s*)lea(\s+)(-?\d+|0[xX][0-9a-fA-F]+)\((%a[0-7]|%sp)\),\s*(%a[0-7]|%sp)', line)
    if match:
        aN = match.group(4)
        if aN == match.group(5):
            val = parseConstantSigned(match.group(3), 8)
    ...
    if IS_ROL_INSTRUCTION_REGEX.match(line) or IS_ROR_INSTRUCTION_REGEX.match(line) or IS_ROXL_INSTRUCTION_REGEX.match(line):
        ...
        # If 1 <= x <= 3
        # rol.b   #4+x,dN   ->   ror.b   #4-x,dN   ; Saves 4*x cycles. Wrong flags
        ...
        # If 1 <= x <= 3
        # ror.b   #4+x,dN   ->   rol.b   #4-x,dN   ; Saves 4*x cycles. Wrong flags
        ...
        # roxl.b  #1,dN     ->   addx.b  dN,dN     ; Saves 4 cycles. Wrong flags
        ...
        # roxl.b  #2,dN     ->   addx.b  dN,dN     ; Saves 2 cycles. Wrong flags
        #                        addx.b  dN,dN
        ...
        # roxl.w  #1,dN     ->   addx.w  dN,dN     ; Saves 4 cycles. Wrong flags
        ...
        # roxl.w  #2,dN     ->   addx.w  dN,dN     ; Saves 2 cycles. Wrong flags
        #                        addx.w  dN,dN
        ...
        # roxl.l  #1,dN     ->   addx.l  dN,dN     ; Saves 2 cycles. Wrong flags
    ...
    if IS_LSL_INSTRUCTION_REGEX.match(line) or IS_ASL_INSTRUCTION_REGEX.match(line):
        ...
        # lsl.b/asl.b   #7,dN   ->   ror.b   #1,dN       ; Saves 4 cycles. Wrong flags
        #                            andi.b  #0x80,dN
        ...
        # lsl.b/asl.b   #8,dN   ->   clr.b   dN          ; Saves 18 cycles. Wrong flags
        ...
        # lsl.w/asl.w   #8,dN   ->   move.b   dN,-(sp)   ; Saves 2 cycles. Wrong flags
        #                            move.w   (sp)+,dN
        #                            clr.b    dN
    ...
    if IS_LSR_INSTRUCTION_REGEX.match(line):
        ...
        # lsr.b   #7,dN   ->   add.b    dN,dN      ; Saves 8 cycles. Wrong flags
        #                      subx.b   dN,dN
        #                      neg.b    dN
        ...
        # lsr.b   #8,dN   ->   clr.b    dN         ; Saves 18 cycles. Wrong flags
        ...
        # lsr.w   #8,dN   ->   move.w   dN,-(sp)   ; Saves 2 cycles. Wrong flags
        #                      clr.w    dN
        #                      move.b   (sp)+,dN
    ...
    if IS_ASR_INSTRUCTION_REGEX.match(line):
        ...
        # If 0 <= x <= 1
        # asr.b   #7+x,dN  ->   add.b    dN,dN     ; Saves 12+2*x cycles. Wrong flags
        #                       subx.b   dN,dN
        ...
        # asr.w   #8,dN    ->   move.w   dN,-(sp)  ; Saves 12+2*x cycles. Wrong flags
        #                       move.b   (sp)+,dN
        #                       ext.w    dN
    ...
    if OPTIMIZE_MULTIPLICATION_HIGH_WORD_IMPORTANT and IS_MUL_INSTRUCTION_REGEX.match(line):
        ...
        if IS_MULS_INSTRUCTION_REGEX.match(line):
            ...
            from optimize_mul_patterns import muls_high_word_important
            return muls_high_word_important(line, i_line, lines, modified_lines)

        # High word of result is important
        if IS_MULU_INSTRUCTION_REGEX.match(line):

            from optimize_mul_patterns import mulu_high_word_important
            return mulu_high_word_important(line, i_line, lines, modified_lines)
    ...
    if OPTIMIZE_MULTIPLICATION_HIGH_WORD_NOT_IMPORTANT and IS_MUL_INSTRUCTION_REGEX.match(line):
        ...
        if IS_MULS_INSTRUCTION_REGEX.match(line):
            ...
            from optimize_mul_patterns import muls_high_word_not_important
            return muls_high_word_not_important(line, i_line, lines, modified_lines)

        # High word of result is NOT important
        if IS_MULU_INSTRUCTION_REGEX.match(line):

            from optimize_mul_patterns import mulu_high_word_not_important
            return mulu_high_word_not_important(line, i_line, lines, modified_lines)
...
def optimize_asm(input_lines: list[str], symbols_filename: str | None, current_pass: int) -> tuple[list[str], int, int]:
    ...
    modified_multi_lines, num_updates_2 = process_single_lines_helper(
        modified_multi_lines, 
    ...
    modified_multi_lines, num_updates_3 = process_single_lines_helper(
        modified_multi_lines, 
    ...
    # Only if at 2nd pass
    if current_pass == 2:
        modified_multi_lines, num_updates_4 = process_single_lines_helper(
            modified_multi_lines, 
    ...
    return (modified_multi_lines, num_updated_lines_found, num_patterns_found)

# %pc@(disp,%xN:s)
gcc_indirection_style_pattern = re.compile(r'%pc@\((-?\d+),%([ad])([0-7]):([bwl])\)')

def convert_from_gcc_indirection_style(line: str) -> str:
    """
...
gcc_inner_displacement_style_pattern = re.compile(
    r'(?:\((-?[0-9a-zA-Z_\.]+(?:\.[bwl])?(?:[\-\+\*]\d+)?),(%a[0-7]|%sp|%pc)\))'  # (label_or_disp[.bwl][+-*N],aN/sp/pc)
    r'|'
    r'(?:\((-?[0-9a-zA-Z_\.]+(?:\.[bwl])?(?:[\-\+\*]\d+)?),(%a[0-7]|%sp|%pc),(%[ad][0-7])(\.[wl])?\))'  # (label_or_disp[.bwl][+-*N],aN/sp/pc,%xN[.s])
)

def convert_from_gcc_inner_displacement_style(line: str) -> str:
    """
    Gets gcc's displacement inside parenthesis:
        (label_or_disp[+-*N],aN/sp/pc)
        (label_or_disp[+-*N],aN/sp/pc,xN.s)
    ...
    def replace_match(match):
        if match.group(1) and match.group(2):
            ...
            s = '' if not match.group(6) else match.group(6)
            return f'{match.group(3)}({match.group(4)},{match.group(5)}{s})'
        # Fallback: return original
        return match.group(0)

    return gcc_inner_displacement_style_pattern.sub(replace_match, line)

gcc_indirection_with_long_dn_access_pattern = re.compile(
    r'(?:'
    r'(?:\((%a[0-7]|%sp|%pc),(%d[0-7])\.l\))'  # (aN/sp/pc,%dN.l)
    r'|'
    r'(?:(-?[0-9a-zA-Z_\.]+(?:[\-\+\*]\d+)?)\((%a[0-7]|%sp|%pc),(%d[0-7])\.l\))'  # label_or_disp[+-*N](aN/sp/pc,%dN.l)
    r')'
)
...
def remove_gcc_dereference_symbol_or_memory(line: str) -> str:
    ...
    """
    # sub(r'\1', line) replaces the entire match with the capturing group 1
    # Note that we omit the last [.wl].
...
def applyGccConversions(lines: list[str]) -> list[str]:
    ...
    for i_line in range(0, len(lines)):
        ...
        # Convert gcc inner displacement style which comes inside parenthesis, into normal style
        line = convert_from_gcc_inner_displacement_style(line)
        # Replace %fp by %a6
...
def remove_simple_abi(lines: list[str]) -> list[str]:
    """
